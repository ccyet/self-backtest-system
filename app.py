...
APP_PAGE_WORKBENCH = "回测工作台"
APP_PAGE_GUIDE = "因子与参数说明"
APP_PAGE_OPTIONS = (APP_PAGE_WORKBENCH, APP_PAGE_GUIDE)
DIRECTION_LABELS = ("做多", "做空")
...
def normalize_direction_label(raw_label: str) -> str:
    return LEGACY_DIRECTION_MAP.get(raw_label, "做多")
...
def render_factor_param_guide_page() -> None:
    section_header(
        "因子与参数说明（人话版）",
        "这页不跑回测，只解释每个选项在做什么、会带来什么结果。",
    )
    st.info("建议先用默认参数跑一遍，再每次只改 1~2 个参数，最容易看出影响。")

    st.markdown("### 先理解：交易方向")
    st.markdown(
        "- **做多**：希望价格上涨后赚钱。\n"
        "- **做空（研究镜像）**：用下跌逻辑做反向研究，不代表可直接实盘融券。"
    )

    st.markdown("### 三类入场因子怎么选")
    with st.expander("1) 跳空（gap）", expanded=True):
        st.markdown(
            "适合抓**开盘直接跳开的行情**。\n"
            "- `跳空幅度`：门槛越大，信号越少但通常更强。\n"
            "- `最大高开/低开过滤`：防止跳得太离谱（常见于追高风险）。\n"
            "- `开仓模式`：\n"
            "  - 严格突破前高/前低：更强调形态突破；\n"
            "  - 开盘相对昨收达阈值：更强调跳空幅度。"
        )

    with st.expander("2) 趋势突破（trend_breakout）", expanded=True):
        st.markdown(
            "适合抓**强趋势继续走强**。\n"
            "- `趋势突破回看天数`：看过去多少天的高/低点当突破线。\n"
            "- 参数越小：更敏感，信号更多但假突破也可能更多。\n"
            "- 参数越大：更稳健，信号更少、通常更晚进场。"
        )

    with st.expander(
        "3) 波动收缩突破（volatility_contraction_breakout）", expanded=True
    ):
        st.markdown(
            "适合抓**先缩量/收敛，再放大突破**。\n"
            "- `收缩区间回看天数`：判断最近是不是进入收缩状态。\n"
            "- `突破回看天数`：确认突破哪一段历史高/低点。\n"
            "- 一般先调收缩，再调突破；两者同时大幅改动不容易定位问题。"
        )

    st.markdown("### 公共风控参数（所有因子都适用）")
    risk_df = pd.DataFrame(
        [
            {
                "参数": "全仓止损（%）",
                "人话解释": "亏到这个比例就直接退出，保护本金。",
                "调大/调小的效果": "调小更保守；调大更能扛波动，但回撤会变大。",
            },
            {
                "参数": "固定止盈（%）",
                "人话解释": "赚到目标就落袋。",
                "调大/调小的效果": "调小更容易止盈；调大可能吃到更大行情但回吐也更常见。",
            },
            {
                "参数": "盈利回撤（%）",
                "人话解释": "先赚过，再回撤到阈值就退出。",
                "调大/调小的效果": "调小更早锁利润；调大更“放手”，但可能回撤更深。",
            },
            {
                "参数": "最多持有天数 N",
                "人话解释": "超过 N 天还没结束，就按时间规则处理。",
                "调大/调小的效果": "调小偏短线；调大偏波段。",
            },
            {
                "参数": "买入/卖出滑点（%）",
                "人话解释": "模拟实际成交比理想价更差。",
                "调大/调小的效果": "调大后结果更保守，更接近真实成交摩擦。",
            },
        ]
    )
    dataframe_stretch(risk_df, hide_index=True, height=280)

    st.markdown("### 参数扫描怎么配（避免组合爆炸）")
    st.markdown(
        "- 先开 **1 维扫描**，确认方向后再开 2 维。\n"
        "- 每次只扫当前因子的核心参数，不要把所有参数一起扫。\n"
        "- `最大组合数` 先控制在 20~40，避免等待太久。\n"
        "- 看结果不要只盯第 1 名，优先找“附近参数都还行”的稳定区间。"
    )
...
active_page = st.sidebar.radio(
    "页面",
    options=list(APP_PAGE_OPTIONS),
    index=0,
    label_visibility="collapsed",
)
if active_page == APP_PAGE_GUIDE:
    render_factor_param_guide_page()
    st.stop()
...
st.session_state["direction_label"] = normalize_direction_label(
    str(st.session_state.get("direction_label", "做多"))
)
...
summary_cols[0].metric(
    "交易方向",
    normalize_direction_label(str(st.session_state.get("direction_label", "做多"))),
)
...
with st.expander("⚙️ 核心交易规则配置", expanded=True):
    ...
    with core_entry_col:
        ...
        if entry_factor == "gap":
            st.caption("做多=向上跳空，做空=向下跳空。")
            entry_top_cols = st.columns([1, 1.6])
            direction_label = entry_top_cols[0].selectbox(
                "交易方向", options=list(DIRECTION_LABELS), key="direction_label"
            )
...
if submitted:
    ...
    params = AnalysisParams(
        ...
        stock_codes=normalize_stock_codes(stock_scope_text),
        gap_direction="up" if direction_label == "做多" else "down",
